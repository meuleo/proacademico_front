    ...
    <div class="container-fluid">
      ...
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="page-title mb-0">
          <Voltar/> Direção/Pedagógica/Disciplinas
        </h4>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-md-6 mb-3 d-flex justify-content-md-end">
          <router-link to="/disciplinas/create">
      ...
      <div v-if="disciplinas.length == 0 && !loader" class="text-center py-3">
        Nenhuma disciplina encontrada
      </div>

      <div v-if="disciplinas.length > 0 && !loader" class="table-responsive">
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="item in disciplinas" :key="item.id">
              ...
              <!-- <td>
                <span  :style="item.nivel == 3 ? { background: '#aaa', padding: '5px 10px', borderRadius: '8px' } : {}">{{ getNivel(item.nivel) }}</span>
              </td> -->
              <td>{{ item.domineo }}</td>
              <td><i :class="item.icon"></i></td>
              <td class="table-actions text-center">
                <div style="width: 200px;">
                  <!-- <router-link :to="`/disciplinas/show/${item.id}`">
                    <button class="btn btn-sm btn-outline-primary" title="Ver">
                      <i class="fas fa-eye"></i>
                    </button>
                  </router-link> -->
                  <router-link :to="`/disciplinas/edit/${item.id}`">
...
export default {
  ...
  methods: {
    ...
    async fetchData(url = request.disciplinasListar) {
      this.loader = true
      try {
    ...
    async confirmarExclusao(id) {
      if (confirm('Tem certeza que deseja excluir esta disciplina?')) {
        try {
          await api.destroy(`disciplinas/destroy/${id}`)
