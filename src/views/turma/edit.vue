  ...
  <div class="main-content" id="mainContent">
    <div class="container-fluid">
      ...
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h4>
            <Voltar/>
            Direção/Pedagógica/Turmas/Editar Turma
          </h4>
      </div>
        ...
        <form @submit.prevent="submitForm">
          <div class="row">
            <div class="mb-3 col-md-6">
              ...
              <select name="" id="" class="form-control" v-model="form.ano_letivo" required>
                <option :value="form.ano_letivo">{{ form.ano_letivo }}</option>
              </select>
            </div>

            <div class="mb-3 col-md-6">
              <label for="sub_sistema" class="form-label">Niveis de Ensino</label>
              <select v-model="sub_sistema" class="form-select" id="sub_sistema" required>
          ...
          <div class="row">
            ...
            <div class="mb-3 col-md-3">
              ...
              <input v-model="form.capacidade" type="number" class="form-control" />
            </div>
            <div class="mb-3 col-md-6">
              <label class="form-label">Diretor da Turma</label>
...
export default {
  ...
  data() {
    return {
      cursos: [],
      series: Array.from({ length: 13 }, (_, i) => String.fromCharCode(65 + i)),
      professores: [],
      form: {
        curso_id: '',
        classe: '',
  ...
  methods: {
    ...
    async carregarTurma() {
      ...
      try {
        const response = await api.show(`${request.turmasShow}${this.turmaId}`)
        this.form = response

        // Inferir sub_sistema com base na classe ou curso, se necessário
        if (this.form.curso_id) {
          this.sub_sistema = 3
        } else if (this.form.classe <= 6) {
          this.sub_sistema = 1
        } else if (this.form.classe >= 7 && this.form.classe <= 9) {
          this.sub_sistema = 2
        } else {
          this.sub_sistema = 0
        }
    ...
    async submitForm() {
      ...
      try {
        await api.update(`${request.turmasUpdate}${this.turmaId}`, this.form)
