  ...
  <div class="main-content" id="mainContent">
    <div class="container-fluid">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-user-plus me-2"></i>Cadastro de Funcionário</h2>
        <a href="lista-funcionarios.html" class="btn btn-outline-secondary">
      ...
      <div class="form-container animate-fadeInUp">
        <form @submit.prevent="submitForm" class="bg-white p-4 rounded shadow-sm">
          ...
          <h4>Função</h4>
          <div class="row mb-3">
            <div class="col-md-3 form-check" v-for="item in cargos" :key="item.id">
              <input class="form-check-input" type="checkbox" :id="'cargo_' + item.id" :value="item.id" v-model="form.cargosSelecionados">
              <label class="form-check-label" :for="'cargo_' + item.id">{{ item.nome }} </label>
            </div>
          </div>
          ...
          <h4>Disciplinas</h4>
          <div class="row mb-3">
            <div class="col-md-3 form-check" v-for="item in disciplinas" :key="item.id">
              <input class="form-check-input" type="checkbox" :id="'disciplina_' + item.id" :value="item.id" v-model="form.disciplinasSelecionadas">
              <label class="form-check-label" :for="'disciplina_' + item.id">{{ item.nome }}</label>
            </div>
          </div>
