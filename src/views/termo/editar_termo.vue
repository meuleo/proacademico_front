<template>
  <div>
    ...
    <div class="main-content p-4">
      <h3>
        <i class="fas fa-arrow-left me-2" @click="$router.back" style="cursor: pointer; font-size: 16px; position: relative; top: -4px;"></i> Editar termo</h3>

      <!-- Configurações -->
      <div class="config mb-3">
        <div class="row p-0">
          <div>
            <p class="mb-1"><strong>Aluno: </strong> {{ nome_completo }}</p>
...
export default {
  ...
  methods: { 
    async onVisualizar() {
      const termo_id = this.$route.params.termo_id || 'nd'
      const alunoId = this.$route.params.alunoId || 'nd'
      const classe = this.$route.params.classe || 'nd'
      ...
      this.$router.push(`/termos/${alunoId}/teste/${nivel}/${classe}/show/${termo_id}`)
    },
    ...
    async loadTermo() {
      try {
        ...
        // Preenche campos básicos (normalizando tipos)
        this.nome_completo = termo.aluno?.nome_completo || (termo.aluno_nome || '');
        ...
        // disciplinas únicas vindas das notas (garante que o id seja number)
        const disciplinasUnicas = termo.notas
          .map(n => n.disciplina || null)
          .filter(d => d && typeof d.id !== 'undefined')
          .filter((d, i, arr) => arr.findIndex(x => Number(x.id) === Number(d.id)) === i)
          .map(d => ({ ...d, id: Number(d.id) })); // garante id numérico
        if (disciplinasUnicas.length) this.disciplinas = disciplinasUnicas;

        // ativa NPP se houver pelo menos uma nota do tipo npp
        this.ativarNpp = termo.notas.some(n => String(n.tipo_nota).toLowerCase() === 'npp');

        // ativa exame/recurso se existirem notas correspondentes
        this.ativarExame = termo.notas.some(n => ['exame','mfe'].includes(String(n.tipo_nota).toLowerCase()));
        this.ativarRecurso = termo.notas.some(n => ['recurso','mfr'].includes(String(n.tipo_nota).toLowerCase()));

        // agrupa notas por disciplina (normalizando trimestre e disciplina_id)
        const rowsByDisc = {};
        termo.notas.forEach(n => {
          if (!n) return;

          const didRaw = n.disciplina_id ?? (n.disciplina && n.disciplina.id) ?? null;
          ...
          // garantir trimestre como número (ou null)
          const tRaw = n.trimestre;
          ...
          // normaliza tipo_nota
          const tipo = String(n.tipo_nota || '').toLowerCase();

          if (['mac','npp','npt','mt'].includes(tipo) && (t === 1 || t === 2 || t === 3)) {
            // escreve no campo apropriado
        ...
        // transforma em array e força disciplina_id como number para o v-model.number do select
        this.rows = Object.values(rowsByDisc).map(r => ({ ...r, disciplina_id: Number(r.disciplina_id) }));

        // recalcula MTs/MFDs/MFE/MFR para consistência
        this.rows.forEach(r => this.calcAllMts(r));
