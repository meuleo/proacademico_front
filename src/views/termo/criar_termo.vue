<template>
  <div>
    ...
    <div class="main-content p-4">
      <h3><i class="fas fa-arrow-left me-2" @click="$router.back" style="cursor: pointer; font-size: 16px; position: relative; top: -4px;"></i> Criar termo</h3>
      ...
      <div class="config mb-3">
        <div class="row p-0">
          <div>
            ...
            <p><strong>Classe: </strong> {{$route.params.classe || ''}}ª Classe</p>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-3 mb-3">
              ...
              <select name="" id="" class="form-select">
                <option value="">Selecione o curso</option>
                <option v-for="item in series" :value="item">
                  {{ item }}
                </option>
              </select>
            </div>
            <div class="col-md-3 p-0 mb-3">
              ...
              <input type="number" v-model="anoLetivo" class="form-control" />
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      nivel: 3,
      series: Array.from({ length: 26 }, (_, i) =>
  ...
  async created() {
    this.alunoId = this.$route.params.aluno_id
    this.classe = this.$route.params.classe
  ...
  methods: {
    ...
    onNotaChange(row, trimestre) {
      if (trimestre) {
        ...
        // No entanto, caso o professor mude notas, chamamos calcMfd também de forma segura
        this.calcMfd(row);
      }
      // sempre recalc MFE e MFR (se aplicáveis)
      this.calcMfe(row);
      this.calcMfr(row);
    },
    ...
    // MFE = média entre MFD e Exame (apenas se exame preenchido)
    calcMfe(row) {
    ...
    async saveTerm() {
      ...
      try {
        ...
        console.error(err);
        this.loaderSave = false
        // alert('Erro ao salvar termo. Verifica console para mais detalhes.');
      }
    },
  }
};
